      ...
      <section className="relative overflow-hidden border-b border-slate-200 bg-[#edf6ff]">
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-20 sm:px-8 sm:py-24 lg:py-28">
          <div className="max-w-4xl">
            ...
            <div className="mt-8 flex flex-wrap gap-x-6 gap-y-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500"><span>NEXUS · Understand</span><span>VERA · Evaluate</span><span>CHAKRA · Defend</span></div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white">
        <div className="mx-auto grid max-w-7xl gap-12 px-6 py-16 sm:px-8 sm:py-20 lg:grid-cols-[0.72fr_1.28fr] lg:gap-16 lg:py-24">
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_28px_70px_-40px_rgba(15,23,42,0.45)] sm:p-8 lg:p-9">
            <div className="border-b border-slate-200 pb-6"><p className="font-mono text-xs font-semibold uppercase tracking-[0.18em] text-violet-700">Let&apos;s talk</p><h2 className="mt-3 text-2xl font-semibold text-slate-950">Tell us how we can help.</h2><p className="mt-2 text-sm leading-6 text-slate-500">Share enough context for us to make the first session useful.</p></div>
            {status === "success" ? (
              <div className="flex flex-col items-center py-16 text-center"><span className="flex h-14 w-14 items-center justify-center rounded-full bg-emerald-50 text-emerald-600"><CheckCircle2 className="h-7 w-7" /></span><h2 className="mt-5 text-xl font-semibold text-slate-950">Message sent</h2><p className="mt-2 max-w-sm text-sm leading-relaxed text-slate-500">We will follow up at the email you entered within 1 business day.</p><Button className="mt-6 rounded-full" variant="outline" onClick={() => setStatus("idle")}>Send another message</Button></div>
            ) : (
