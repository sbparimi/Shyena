    ...
    <main className="overflow-hidden bg-white text-slate-950">
      <section className="relative overflow-hidden border-b border-slate-200 bg-[#edf6ff]">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-6 py-16 sm:py-20 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:gap-16 lg:px-8 lg:py-24">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-violet-200 bg-white/70 px-3.5 py-2 text-[11px] font-bold uppercase tracking-[0.18em] text-violet-700 shadow-sm"><span className="h-1.5 w-1.5 rounded-full bg-violet-500" /> AI agent assurance</div>
            <h1 className="mt-7 max-w-2xl text-5xl font-semibold leading-[0.98] tracking-[-0.045em] text-slate-950 sm:text-6xl lg:text-[70px]">Test your AI agents before they reach production.</h1>
            <p className="mt-7 max-w-xl text-lg leading-8 text-slate-600 sm:text-xl">Shyena turns Cognigy flows into executable journeys, evaluates real conversations, finds security risks, and gives teams evidence to release with confidence.</p>
            <div className="mt-9 flex flex-wrap gap-3"><Link to="/contact" className="inline-flex items-center gap-2 rounded-full bg-[#f6b800] px-6 py-3.5 text-sm font-extrabold uppercase tracking-[0.01em] text-slate-950 shadow-sm transition hover:bg-[#e9aa00]">Let's talk <ArrowRight className="h-4 w-4" /></Link><a href="#how-it-works" className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white/80 px-6 py-3.5 text-sm font-semibold text-slate-800 transition hover:border-slate-400"><Play className="h-4 w-4" /> See how it works</a></div>
      ...
      <section className="bg-[#f7f8fb]"><div className="mx-auto max-w-7xl px-6 py-20 lg:px-8 lg:py-24"><div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end"><div className="max-w-3xl"><div className="text-xs font-bold uppercase tracking-[0.2em] text-violet-600">One AI agent. Three assurance layers.</div><h2 className="mt-4 text-4xl font-semibold tracking-[-0.03em] text-slate-950 sm:text-5xl">Understand. Test. Defend.</h2><p className="mt-5 text-lg leading-8 text-slate-600">Three products work as one assurance system. Each stage adds evidence to the same release decision.</p></div><Link to="/pricing" className="inline-flex shrink-0 items-center gap-2 text-sm font-semibold text-slate-900">View platform pricing <ArrowRight className="h-4 w-4"/></Link></div><div className="mt-12 grid gap-5 lg:grid-cols-3">{products.map(product=><ProductCard key={product.name} product={product}/>)}</div></div></section>
