      ...
      <section className="relative overflow-hidden bg-lavender text-lavender-foreground">
        ...
        <div className="relative mx-auto max-w-7xl px-5 pb-24 pt-20 sm:px-8 sm:pb-32 sm:pt-28">
          <div className="mx-auto max-w-4xl text-center">
            <span className="glass-card inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-medium text-muted-foreground"><Sparkles className="h-3.5 w-3.5 text-primary" />AI Agent Assurance Platform</span>
            <h1 className="mt-7 text-5xl leading-[1.03] tracking-tight sm:text-7xl">Know if your AI agent is <span className="text-gradient-brand">ready for production.</span></h1>
            <p className="mx-auto mt-7 max-w-2xl text-lg leading-relaxed text-lavender-foreground/75 sm:text-xl">Test real conversations, evaluate behavior, uncover security risks, and trace every result back to evidence. Shyena turns agent quality into a release decision you can defend.</p>
            <div className="mt-9 flex flex-wrap justify-center gap-3"><Button asChild size="lg"><Link to="/contact">Request a Demo <ArrowRight className="h-4 w-4" /></Link></Button><Button asChild size="lg" variant="ghost"><a href="#assurance">See How It Works</a></Button></div>
          </div>
          <Reveal className="mx-auto mt-16 max-w-5xl"><div className="glass-card rounded-3xl border border-border/60 bg-background/85 p-4 shadow-2xl backdrop-blur sm:p-6"><div className="grid gap-4 md:grid-cols-5">{STEPS.map(([number, title, body]) => <div key={number} className="rounded-2xl border border-border/60 bg-card p-5"><div className="text-xs font-semibold tracking-widest text-primary">{number}</div><div className="mt-3 text-lg font-semibold">{title}</div><p className="mt-2 text-sm leading-relaxed text-muted-foreground">{body}</p></div>)}</div><div className="mt-4 rounded-2xl border border-primary/20 bg-primary/5 p-5 text-center text-sm font-medium">Every verdict has a reason. Every reason has evidence.</div></div></Reveal>
        </div>
      </section>
