  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Chakra — Continuous Assurance for Agentic AI — Shyena" },
      {
        ...
        content: "Discover. Predict. Attack. Verify. Govern. End-to-end security assurance for agentic AI systems.",
      },
...
const PIPELINE = [
  ...
  { title: "Govern", body: "A compositional assurance score gates releases in CI/CD; critical findings always block, no exceptions." },
] as const;
  ...
  return (
    <>
        ...
        <div className="relative mx-auto w-full max-w-7xl px-5 pb-4 pt-20 sm:px-8 sm:pt-28">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h1 className="mt-6 text-4xl font-bold leading-[1.08] sm:text-6xl">
              Continuous assurance for{" "}
              <span className="text-gradient-brand">agentic AI systems.</span>
            </h1>
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-muted-foreground">
              Chakra discovers every tool, permission and trust boundary your agents have, predicts how
              the system can fail, and attacks it end to end — orchestrating Ziran, Garak and its own
              AI Red Team under one roof. Every finding is verified with full evidence, not a bare
              vulnerability flag, and only releases with proven-safe outcomes are allowed through.
            </p>
      ...
      <section className="mx-auto w-full max-w-7xl px-5 pb-4 sm:px-8">
        ...
        >
          <img
            ...
            alt="A graph of agentic AI risk categories — goal hijack, tool misuse, privilege abuse, memory poisoning — with dangerous paths flagged in red, converging into excessive agency, the kind of compositional risk Chakra's digital twin surfaces"
        ...
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-navy-muted">
              Five stages, each producing evidence the next is allowed to trust — from a live model of
              your agent's capabilities to a release verdict your team can defend.
            </p>
          </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-5 py-24 sm:px-8">
        <div className="max-w-2xl">
          <p className="font-mono text-xs uppercase tracking-[0.18em] text-primary">What makes it different</p>
          <h2 className="mt-4 text-3xl font-bold sm:text-4xl">A control plane, not another point scanner.</h2>
          <p className="mt-4 text-muted-foreground">
            Point tools test the model or a single prompt. Chakra tests the entire agentic
            system — tools, memory, workflows and multi-agent protocols — end to end.
          </p>
        </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-5 pb-24 sm:px-8">
        <div className="rounded-3xl border border-navy-border bg-navy p-8 sm:p-12">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1.3fr_1fr]">
            <div className="rounded-xl border border-navy-border bg-white/[0.03] p-6 font-mono text-xs leading-relaxed text-navy-muted sm:text-sm">
              <p className="text-navy-foreground">CHK-2026-001245 · Critical</p>
              <p className="mt-2">Attack: Indirect Prompt Injection → Customer PII Exfiltration</p>
              <p className="mt-1">
                Path: Internet → WebService → ResearchAgent → PlannerAgent → CRM → CustomerDB
              </p>
              <ul className="mt-3 space-y-1">
                <li>[1] Malicious webpage served payload</li>
                <li>[2] Agent retrieved poisoned instruction</li>
                <li>[3] Planner changed objective to "dump customer data"</li>
                <li>[4] CRM query executed — 1,248 records</li>
                <li>[5] External webhook request with PII</li>
              </ul>
              <p className="mt-3 text-destructive">Release Gate: BLOCK (confirmed data leakage)</p>
            </div>
            <div>
              <p className="text-sm leading-relaxed text-navy-muted">
                Every finding reconstructs the complete chain from input to impact, with screenshots,
                transcripts, API logs and memory diffs behind each step. If an attack stalls partway,
                Chakra records exactly where — a guardrail, a missing permission — rather than
                reporting a false alarm. A confirmed exploit is automatically minimized into a
                regression test that runs in every future build, so the same hole never reopens.
              </p>
              <Button
                asChild
                variant="outline"
                className="mt-6 border-navy-border bg-transparent text-navy-foreground hover:bg-white/10 hover:text-navy-foreground"
              >
                <Link to="/contact">See a Replay on Your Own Agent</Link>
              </Button>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-5 pb-24 sm:px-8">
        <div className="max-w-2xl">
          <p className="font-mono text-xs uppercase tracking-[0.18em] text-primary">No silent failures</p>
          <h2 className="mt-4 text-3xl font-bold sm:text-4xl">Compositional, not a single vanity score.</h2>
          <p className="mt-4 text-muted-foreground">
            Sub-scores show where the weakness actually is — but any confirmed critical exploit
            overrides the aggregate and blocks release, regardless of how healthy the rest looks.
          </p>
        </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-5 pb-24 sm:px-8">
        <div className="max-w-2xl">
          <p className="font-mono text-xs uppercase tracking-[0.18em] text-primary">Where Chakra fits</p>
          <h2 className="mt-4 text-3xl font-bold sm:text-4xl">Orchestrates the toolchain, doesn't replace it.</h2>
          <p className="mt-4 text-muted-foreground">
            Chakra's strategy isn't to out-build Ziran or Garak — it incorporates them, and adds the
            missing pieces: a comprehensive digital twin, autonomous attack planning, business policy
            testing, evidence tracking and release governance.
          </p>
        </div>
        <div className="mt-10 overflow-x-auto rounded-2xl border border-border bg-card shadow-card">
          <div className="min-w-[640px]">
            <div className="grid grid-cols-[2fr_repeat(5,1fr)] items-center gap-2 border-b border-border bg-secondary/40 px-6 py-4 text-sm font-semibold">
              <span className="text-foreground">Capability</span>
              {COMPARE_COLUMNS.map((col, i) => (
                <span key={col} className={`text-center ${i === COMPARE_COLUMNS.length - 1 ? "text-primary" : "text-muted-foreground"}`}>
                  {col}
                </span>
              ))}
            </div>
            {COMPARE_ROWS.map((row, i) => (
              <div
                key={row.feature}
                className={`grid grid-cols-[2fr_repeat(5,1fr)] items-center gap-2 px-6 py-4 text-sm last:rounded-b-2xl ${i % 2 === 1 ? "bg-secondary/20" : ""}`}
              >
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          Ziran remains an independent open-source project Shyena has no role in developing — Chakra
          integrates it as one of several attack engines rather than replacing it.
        </p>
      </section>
      ...
      <section className="mx-auto w-full max-w-7xl px-5 pb-24 sm:px-8">
        <div className="overflow-hidden rounded-3xl border border-navy-border bg-navy px-8 py-16 text-center shadow-elevated sm:px-16">
          ...
          <p className="mx-auto mt-4 max-w-xl text-navy-muted">
            We'll run a scoped Chakra campaign against your live agent and walk through the digital
            twin, the attack chain, and the release verdict with you.
          </p>
          <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
            <Button asChild size="lg">
              <Link to="/contact">
