...
const customers = [
  ["PostNL", "Conversational AI assurance"],
  ["A Luxembourg-based bank", "AI assurance in a regulated environment"],
  ["Crossover", "AI quality and assurance"],
] as const;
...
function AboutPage() {
  return <main className="overflow-hidden bg-white text-slate-950">
    ...
    <section className="border-b border-slate-300 bg-[#0b0920] text-white"><div className="mx-auto max-w-[1160px] px-6 py-14 sm:px-10 lg:px-12 lg:py-16"><div className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-[#ffb703]">Customer proof</div><h2 className="mt-4 max-w-3xl font-[Sora] text-[clamp(2.3rem,4vw,3.8rem)] font-extrabold leading-[0.98] tracking-[-0.05em]">Three customer environments. One assurance mission.</h2><p className="mt-6 max-w-2xl text-lg leading-8 text-white/60">Shyena currently works with customer teams across logistics, banking and technology. We are building verified customer outcomes before making claims about scale.</p><div className="mt-10 grid gap-px bg-white/10 md:grid-cols-3">{customers.map(([name,context],index) => <div key={name} className="bg-[#0b0920] p-7 sm:p-8"><div className="font-mono text-xs text-white/35">0{index + 1}</div><h3 className="mt-5 text-xl font-extrabold text-white">{name}</h3><p className="mt-2 text-sm leading-6 text-white/55">{context}</p></div>)}</div></div></section>
