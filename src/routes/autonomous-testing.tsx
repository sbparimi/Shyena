    ...
    <div className="overflow-hidden rounded-[2px] border border-slate-700 bg-[#0b0920] shadow-[0_30px_80px_-50px_rgba(11,9,32,.7)]">
      ...
      <div className="grid border-t border-white/10 sm:grid-cols-[1fr_auto] sm:items-center">
        <div className="px-6 py-5 sm:px-8">
          ...
          <div className="mt-2 flex flex-wrap gap-x-5 gap-y-2 text-[9px] font-semibold text-white/45">
            <span>Evidence collected</span><span>Requirement evaluated</span><span>Failure classified</span><span>Policy evaluated</span>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1240px] px-6 pb-20 pt-16 sm:px-8 lg:px-10 lg:pb-28 lg:pt-24">
          <div className="grid gap-14 lg:grid-cols-[1.1fr_.9fr] lg:items-end lg:gap-20">
            <div>
              ...
              <h1 className="max-w-4xl font-[Sora] text-[clamp(3rem,6.2vw,6.6rem)] font-extrabold leading-[.9] tracking-[-.07em]">
                Release faster.<br />
                <span className="text-slate-400">Know what is safe.</span>
              </h1>
              <p className="mt-7 max-w-2xl text-base leading-7 text-slate-600 sm:text-lg sm:leading-8">
                Software changes faster than traditional testing can keep up. Shyena continuously connects <strong className="text-slate-950">change, risk, testing and evidence</strong> so engineering leaders can make release decisions with confidence.
              </p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Link to="/contact" className="inline-flex h-12 items-center gap-2 bg-[#ffb703] px-6 text-sm font-extrabold">See how it works <ArrowRight className="h-4 w-4" /></Link>
                <a href="#problem" className="inline-flex h-12 items-center gap-2 border border-slate-300 px-6 text-sm font-semibold">The problem we solve <ArrowRight className="h-4 w-4" /></a>
              </div>
            </div>
            ...
            <div className="border-l-2 border-[#ffb703] pl-6 lg:mb-1">
              ...
              <p className="mt-4 font-[Sora] text-2xl font-bold leading-tight tracking-[-.035em] text-slate-950 sm:text-3xl">
                “A test failed. Should we stop the release?”
              </p>
              <p className="mt-4 text-sm leading-6 text-slate-500">
                Traditional automation reports a test result. Shyena determines what the failure means for the requirement, the customer and the release.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1240px] px-6 py-16 sm:px-8 lg:px-10 lg:py-24">
          <div className="grid gap-12 lg:grid-cols-[.72fr_1.28fr] lg:gap-24">
            <div>
              ...
              <h2 className="font-[Sora] text-[clamp(2.4rem,4.3vw,4.4rem)] font-extrabold leading-[.92] tracking-[-.065em]">Testing has become the bottleneck.</h2>
              <p className="mt-6 max-w-md text-sm leading-6 text-slate-500">More software change creates more test selection, more maintenance and more failure investigation. The result is slower releases without proportional increases in confidence.</p>
            </div>
            <div>
      ...
      <section className="border-b border-slate-200">
        <div className="mx-auto max-w-[1240px] px-6 py-16 sm:px-8 lg:px-10 lg:py-24">
          <div className="max-w-3xl">
            <Eyebrow>The Shyena system</Eyebrow>
            <h2 className="font-[Sora] text-[clamp(2.5rem,4.7vw,4.8rem)] font-extrabold leading-[.9] tracking-[-.065em]">From change to<br /><span className="text-slate-400">release confidence.</span></h2>
            <p className="mt-5 max-w-2xl text-sm leading-6 text-slate-500">Shyena operates as an intelligence layer around the quality infrastructure you already have. It decides what matters, coordinates execution and turns runtime evidence into a business-level conclusion.</p>
          </div>

          <div className="mt-14 grid gap-3 md:grid-cols-5">
            {[
              ["01", "UNDERSTAND", "Read requirements, code, dependencies and history.", Search],
              ["02", "ASSESS", "Map change to impact, risk and coverage.", Target],
              ["03", "TEST", "Execute the right assurance through existing engines.", Workflow],
              ["04", "INVESTIGATE", "Correlate failures and identify what actually broke.", Layers3],
              ["05", "PROVE", "Verify requirements and produce an evidence-backed decision.", ShieldCheck],
            ].map(([number, title, description, Icon]) => (
              <div key={String(number)} className="shyena-step border border-slate-200 bg-white p-5">
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[9px] font-bold tracking-[.16em] text-slate-400">{String(number)}</span>
                  <Icon className="h-4 w-4 text-slate-300" />
                </div>
                <div className="mt-12 font-mono text-[9px] font-bold tracking-[.16em]">{String(title)}</div>
                <p className="mt-3 text-xs leading-5 text-slate-500">{String(description)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex flex-wrap justify-between gap-4 font-mono text-[8px] font-bold uppercase tracking-[.16em] text-slate-400">
            <span>Change intelligence</span><span>Risk-based selection</span><span>Evidence collection</span><span>Forensic verification</span><span>Release governance</span>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#f8fafc]">
        <div className="mx-auto max-w-[1240px] px-6 py-16 sm:px-8 lg:px-10 lg:py-24">
          <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr] lg:items-end lg:gap-20">
            <div>
              <Eyebrow>What changes for leadership</Eyebrow>
              <h2 className="font-[Sora] text-[clamp(2.4rem,4vw,4rem)] font-extrabold leading-[.92] tracking-[-.06em]">A test result becomes a decision.</h2>
              <p className="mt-5 text-sm leading-6 text-slate-500">Instead of asking engineers to interpret hundreds of signals, leadership receives the conclusion and the evidence behind it.</p>
            </div>
            <ReleaseDecision />
          </div>
        </div>
      ...
      <section className="border-y border-slate-200">
        <div className="mx-auto max-w-[1240px] px-6 py-16 sm:px-8 lg:px-10 lg:py-24">
          <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:gap-24">
            <div>
              <Eyebrow>Why autonomous is different</Eyebrow>
              <h2 className="font-[Sora] text-[clamp(2.4rem,4vw,4.1rem)] font-extrabold leading-[.92] tracking-[-.06em]">Automation executes.<br /><span className="text-slate-400">Shyena reasons.</span></h2>
            </div>
            <div className="grid gap-0 sm:grid-cols-2 sm:border-t sm:border-slate-200">
      ...
      <section>
        <div className="mx-auto max-w-[1240px] px-6 py-16 sm:px-8 lg:px-10 lg:py-24">
          <div className="grid gap-14 lg:grid-cols-[.65fr_1.35fr] lg:gap-24">
            <div>
              <Eyebrow>Works with your environment</Eyebrow>
              <h2 className="font-[Sora] text-[clamp(2.3rem,4vw,4rem)] font-extrabold leading-[.92] tracking-[-.06em]">Keep the tools.<br /><span className="text-slate-400">Add the intelligence.</span></h2>
              <p className="mt-5 max-w-md text-sm leading-6 text-slate-500">Shyena does not require a replacement testing stack. It connects the systems that already contain requirements, code, automation and runtime evidence.</p>
              <div className="mt-7 flex flex-wrap gap-2">{stack.map((item) => <span key={item} className="border border-slate-200 px-3 py-2 font-mono text-[8px] font-bold uppercase tracking-[.12em] text-slate-500">{item}</span>)}</div>
            </div>
      ...
      <section className="bg-[#0b0920] text-white">
        <div className="mx-auto max-w-[1240px] px-6 py-16 sm:px-8 lg:px-10 lg:py-24">
          <div className="max-w-4xl">
            ...
            <p className="mt-7 max-w-2xl text-base leading-7 text-white/45">Shyena makes quality engineering autonomous without removing human accountability. AI reasons. Deterministic systems verify. Evidence proves. Policy governs. Leaders decide.</p>
            <Link to="/contact" className="mt-8 inline-flex h-12 items-center gap-2 bg-[#ffb703] px-6 text-sm font-extrabold text-slate-950">Discuss your quality system <ArrowRight className="h-4 w-4" /></Link>
          </div>
        </div>
      </section>
    </main>
