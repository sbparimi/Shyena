...
function ResourcesPage() {
  return <main className="overflow-x-hidden bg-white text-[#17213f]">
    ...
    <section className="bg-[#fafbfc]"><div className="mx-auto max-w-[1280px] px-5 py-12 sm:px-8 lg:px-10 lg:py-16"><div className="grid gap-4 lg:grid-cols-3">{cards.map(([type,title,text,cta,Icon]) => <article key={title} className="overflow-hidden rounded-xl border border-[#e0e3e8] bg-white transition hover:-translate-y-0.5 hover:shadow-[0_22px_50px_-34px_rgba(23,33,63,.4)]"><div className="flex h-40 items-center justify-center bg-[#f4f6fa]"><Icon className="h-12 w-12 text-[#ff5a0a]" /></div><div className="p-6"><div className="text-[10px] font-bold uppercase tracking-[.14em] text-[#e87512]">{type}</div><h2 className="mt-3 text-xl font-extrabold text-[#17213f]">{title}</h2><p className="mt-2 text-sm leading-6 text-[#69707d]">{text}</p><Link to="/blog" className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-[#ff5a0a]">{cta}<ArrowRight className="h-4 w-4" /></Link></div></article>)}</div></div></section>
  </main>;
}
