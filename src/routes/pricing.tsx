...
function PricingCalculator() {
  ...
  const estimate = useMemo(() => {
    const flowCount = parseInteger(flows);
    const conversationCount = parseInteger(conversations);
    const securityCount = parseInteger(security);
    const flowCost = flowCount * RATE;
    const conversationCost = conversationCount * RATE;
    const securityCost = securityCount * RATE;
    return { flowCost, conversationCost, securityCost, monthly: flowCost + conversationCost + securityCost };
  }, [flows, conversations, security]);

  const field = (label: string, value: string, setter: (value: string) => void, note: string, id: string) => {
    const increment = () => setter(String(parseInteger(value) + 1));
    const decrement = () => setter(String(Math.max(0, parseInteger(value) - 1)));
    ...
    return <div>
      ...
      <div className="relative mt-2">
        <input
          ...
          onBlur={() => setter(String(parseInteger(value)))}
          aria-describedby={`${id}-note`}
          className="h-16 w-full rounded-2xl border border-slate-300 bg-white px-5 pr-20 text-2xl font-semibold tabular-nums text-slate-950 outline-none transition-all placeholder:text-slate-300 focus:border-violet-500 focus:ring-4 focus:ring-violet-100"
          aria-label={`${label}, whole numbers only`}
        />
        <div className="absolute inset-y-2 right-2 flex w-10 flex-col overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
          <button type="button" onClick={increment} aria-label={`Increase ${label}`} className="flex flex-1 items-center justify-center text-slate-500 transition hover:bg-white hover:text-slate-950 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-violet-500">+</button>
          <div className="border-t border-slate-200" />
          <button type="button" onClick={decrement} aria-label={`Decrease ${label}`} className="flex flex-1 items-center justify-center text-slate-500 transition hover:bg-white hover:text-slate-950 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-violet-500">−</button>
        </div>
      </div>
      <span id={`${id}-note`} className="mt-2 block text-xs font-medium text-slate-500">{note} · Whole numbers only</span>
    </div>;
  };

  return <section id="calculator" className="border-y border-slate-200 bg-slate-50 py-20 sm:py-24"><div className="mx-auto max-w-7xl px-5 sm:px-8"><div className="grid gap-12 lg:grid-cols-[0.8fr_1.2fr] lg:items-start"><div><div className="flex h-11 w-11 items-center justify-center rounded-xl bg-violet-100 text-violet-700"><Calculator className="h-5 w-5" /></div><p className="mt-6 font-mono text-xs uppercase tracking-[0.18em] text-violet-600">Pricing calculator</p><h2 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">Calculate your own monthly usage.</h2><p className="mt-5 text-base leading-relaxed text-slate-600">Enter the activity you expect to generate and execute. The estimate updates instantly as you type using the public €0.05 usage rates.</p><div className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 text-sm text-slate-600"><p className="font-semibold text-slate-950">Three independent usage units</p><p className="mt-2 leading-relaxed">CIS generation, Vera conversations and Chakra security interactions are measured separately. One unit is never converted into another.</p></div></div><div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl sm:p-8"><div className="grid gap-7 md:grid-cols-3">{field("CIS test journeys generated", flows, setFlows, "€0.05 each", "cis-usage")}{field("Vera AI conversations", conversations, setConversations, "€0.05 each", "vera-usage")}{field("Chakra security interactions", security, setSecurity, "€0.05 each", "chakra-usage")}</div><div className="mt-8 grid gap-3 border-t border-slate-200 pt-7 sm:grid-cols-3"><div><p className="text-xs uppercase tracking-wider text-slate-500">CIS</p><p className="mt-1 text-xl font-bold tabular-nums text-slate-950">{formatEuro(estimate.flowCost)}</p></div><div><p className="text-xs uppercase tracking-wider text-slate-500">Vera</p><p className="mt-1 text-xl font-bold tabular-nums text-slate-950">{formatEuro(estimate.conversationCost)}</p></div><div><p className="text-xs uppercase tracking-wider text-slate-500">Chakra</p><p className="mt-1 text-xl font-bold tabular-nums text-slate-950">{formatEuro(estimate.securityCost)}</p></div></div><div className="mt-7 rounded-2xl bg-slate-950 p-6 text-white sm:p-7"><p className="text-xs uppercase tracking-[0.18em] text-slate-400">Estimated monthly usage</p><p className="mt-2 text-4xl font-bold tracking-tight tabular-nums sm:text-5xl">{formatEuro(estimate.monthly)}</p><p className="mt-2 text-xs text-slate-400">Usage estimate only. VAT and separately scoped professional services are excluded.</p></div><div className="mt-6 flex flex-wrap gap-3"><Button asChild><Link to="/contact">Discuss this estimate <ArrowRight className="h-4 w-4" /></Link></Button><a href="#pricing-details" className="inline-flex h-10 items-center rounded-md border border-slate-300 px-4 text-sm font-medium text-slate-700 transition hover:bg-slate-50">See pricing details</a></div></div></div></div></section>;
}
