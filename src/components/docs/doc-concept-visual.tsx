...
const CONCEPTS: Record<string, { kicker: string; message: string; pieces: Array<{ label: string; icon: LucideIcon }> }> = {
  ...
  "SAGE Content Engineering": {
    kicker: "INTERNAL / CONTENT ENGINEERING",
    message: "An internal publishing pipeline for researching, validating and maintaining Shyena documentation and website resources.",
    pieces: [
      ...
      <div className="relative p-5 sm:p-7">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="hidden rounded-full border border-[#a855f7]/20 bg-[#7c3aed]/10 px-2.5 py-1 font-mono text-[9px] uppercase tracking-wider text-[#c4b5fd] sm:block">
            {isInternal ? "internal pipeline" : "agent assembly"}
          </div>
        </div>
        <div className="mt-5 flex flex-wrap gap-2">
          {AGENTS.map(({ label, icon: Icon }) => (
            <div key={label} className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.025] px-2.5 py-1.5 font-mono text-[8px] uppercase tracking-[0.12em] text-[#9f97b3]">
              <Icon className="h-3 w-3 text-[#c4b5fd]" />{label}
            </div>
          ))}
        </div>
        <div className="relative mx-auto mt-5 min-h-[390px] max-w-[620px]">
          ...
          <div className="absolute left-1/2 top-1/2 flex h-28 w-28 -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center rounded-3xl border border-[#a855f7]/40 bg-[#1b1240] shadow-[0_0_35px_rgba(168,85,247,.24)]">
            ...
            <span className="mt-2 font-mono text-[10px] font-bold tracking-[0.18em] text-white">{isInternal ? "SAGE" : "SHYENA"}</span>
            <span className="mt-1 text-[9px] text-[#9f97b3]">{isInternal ? "content pipeline" : "assurance core"}</span>
          </div>
          <svg className="absolute inset-0 h-full w-full" viewBox="0 0 620 390" aria-hidden="true">
            <defs><linearGradient id="doc-flow" x1="0" x2="1"><stop offset="0%" stopColor="#7c3aed" stopOpacity="0.05" /><stop offset="50%" stopColor="#c4b5fd" stopOpacity="0.7" /><stop offset="100%" stopColor="#7c3aed" stopOpacity="0.05" /></linearGradient></defs>
          ...
          {concept.pieces.map(({ label, icon: Icon }, index) => {
            const positions = ["left-1 top-3 sm:left-4", "right-1 top-3 sm:right-4", "left-0 top-1/2 -translate-y-1/2 sm:left-3", "right-0 top-1/2 -translate-y-1/2 sm:right-3", "left-1 bottom-3 sm:left-4", "right-1 bottom-3 sm:right-4"];
            return <div key={label} className={`absolute ${positions[index]} z-10 w-[116px] rounded-2xl border border-white/10 bg-[#15102d]/90 p-3 shadow-xl backdrop-blur-md animate-pulse sm:w-[132px]`} style={{ animationDuration: "3.8s", animationDelay: `${index * 320}ms` }}><div className="flex items-center gap-2"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-[#7c3aed]/15 text-[#c4b5fd]"><Icon className="h-3.5 w-3.5" /></span><span className="min-w-0 truncate text-[10px] font-semibold text-[#e8e3f2] sm:text-xs">{label}</span></div><div className="mt-2 flex items-center gap-1.5 font-mono text-[8px] uppercase tracking-wider text-[#817991]"><span className="h-1 w-1 rounded-full bg-[#a855f7]" />connected</div></div>;
          })}
          <div className="absolute bottom-0 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2 rounded-full border border-white/10 bg-[#0f0a21]/90 px-3 py-1.5 font-mono text-[9px] uppercase tracking-[0.16em] text-[#9f97b3] backdrop-blur-md"><span className="h-1.5 w-1.5 animate-ping rounded-full bg-[#a855f7]" />{isInternal ? "content pipeline → validated publication" : "agents assemble pieces → evidence → decision"}</div>
        </div>
      </div>
    </section>
  );
