...
export function VideoExperience() {
  ...
  useEffect(() => {
    ...
    return () => { music.pause(); music.src = ""; musicRef.current = null; };
  }, []);
  ...
  const togglePlayback = async () => { if (video.paused) await video.play(); else video.pause(); };
  ...
  const toggleMusic = async () => {
    ...
    if (!music) return;
    if (!music.paused) { music.pause(); setMusicEnabled(false); return; }
    try { await music.play(); setMusicEnabled(true); } catch { setMusicEnabled(false); }
  };
  ...
  return createPortal(
    <div className="pointer-events-none absolute inset-0 z-20" onMouseDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()}>
      ...
      <div className="absolute bottom-0 left-0 right-0 p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] sm:p-4 sm:pb-4">
        <div className="pointer-events-auto rounded-2xl border border-white/10 bg-[#090713]/92 p-2.5 text-white shadow-2xl backdrop-blur-xl sm:p-3">
          <div className="relative mb-2 h-1.5 w-full overflow-hidden rounded-full bg-white/15"><div className="absolute inset-y-0 left-0 rounded-full bg-violet-400" style={{ width: `${progress}%` }} /><input aria-label="Video progress" type="range" min={0} max={duration || 0} step={0.1} value={Math.min(currentTime, duration || 0)} onChange={(event) => seek(Number(event.target.value))} className="absolute inset-0 h-full w-full cursor-pointer opacity-0" /></div>
          <div className="flex items-center gap-2 sm:gap-3">
            <button type="button" aria-label={playing ? "Pause video" : "Play video"} onClick={togglePlayback} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white text-[#090713] transition hover:bg-violet-100 focus:outline-none focus:ring-2 focus:ring-violet-300">{playing ? <Pause className="h-4 w-4" fill="currentColor" /> : <Play className="h-4 w-4" fill="currentColor" />}</button>
            <span className="hidden min-w-[74px] font-mono text-[10px] tabular-nums text-white/60 sm:inline">{formatTime(currentTime)} / {formatTime(duration)}</span>
            <button type="button" aria-pressed={musicEnabled} aria-label={musicEnabled ? "Turn background music off" : "Turn background music on"} title={musicEnabled ? "Turn background music off" : "Turn background music on"} data-music-control="true" data-music-active={musicEnabled ? "true" : "false"} onClick={toggleMusic} className="flex h-9 items-center gap-2 rounded-xl px-3 text-xs font-medium transition focus:outline-none focus:ring-2 focus:ring-violet-300"><Music2 className="h-4 w-4" /><span className="hidden sm:inline">Music</span></button>
            <div className="ml-auto" />
            <button type="button" aria-label={fullscreen ? "Exit fullscreen" : "Enter fullscreen"} title={fullscreen ? "Exit fullscreen" : "Enter fullscreen"} onClick={toggleFullscreen} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl text-white/75 transition hover:bg-white/10 hover:text-white focus:outline-none focus:ring-2 focus:ring-violet-300">{fullscreen ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}</button>
          </div>
        </div>
      </div>
    </div>,
