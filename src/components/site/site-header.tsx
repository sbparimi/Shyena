...
export function SiteHeader() {
  ...
  return (
    <header className="relative z-50 border-b border-[#e8eaf0] bg-white" data-shyena-global-header="true">
        ...
        <nav aria-label="Primary navigation" className="hidden flex-1 items-center justify-center gap-7 lg:flex xl:gap-9">
          {NAV.map((item) => (
            <Link
              key={item.to}
              to={item.to}
              className="group inline-flex items-center gap-1.5 whitespace-nowrap text-[14px] font-medium text-[#18213f] transition-colors hover:text-[#e87512]"
            >
              {item.label}
              {item.dropdown && <ChevronDown className="h-3.5 w-3.5 text-[#6b7280] transition-transform group-hover:translate-y-0.5" />}
            </Link>
          ))}
        </nav>
      ...
      {mobileOpen && (
        <div className="border-t border-[#e8eaf0] bg-white px-5 pb-6 pt-3 lg:hidden">
          <nav aria-label="Mobile navigation" className="grid gap-1">
            {MOBILE_NAV.map((item) => <Link key={`${item.label}-${item.to}`} to={item.to} onClick={() => setMobileOpen(false)} className="flex min-h-12 items-center justify-between border-b border-[#eeeeee] px-1 text-[15px] font-medium text-[#17213f]">{item.label}<ArrowRight className="h-4 w-4 text-[#9aa1ae]" /></Link>)}
          </nav>
          <Link to="/contact" onClick={() => setMobileOpen(false)} className="mt-5 flex h-11 items-center justify-center gap-2 rounded-xl bg-[#ff5a0a] text-sm font-semibold text-white">Book a demo <ArrowRight className="h-4 w-4" /></Link>
        </div>
      )}
    </header>
  );
