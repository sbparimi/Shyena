...
function MenuGroup({ items }: { items: readonly { to: string; icon: typeof Gauge; title: string; description: string; kicker?: string }[] }) {
  return (
        ...
        return (
          <li key={item.to}>
            <NavigationMenuLink asChild>
              <Link to={item.to} className="group flex select-none items-start gap-3 rounded-xl p-3.5 leading-none no-underline outline-none transition-colors hover:bg-secondary">
    ...
    <header className="sticky top-0 z-50 border-b border-border/70 bg-background/88 backdrop-blur-2xl">
      <div className="mx-auto flex h-[68px] w-full max-w-7xl items-center justify-between gap-6 px-5 sm:px-8">
                ...
                <NavigationMenuContent>
                  <div className="flex w-[650px] p-3">
                    ...
                    <div className="m-2 hidden w-[190px] flex-col justify-between rounded-xl bg-navy p-4 text-navy-foreground lg:flex">
                      ...
                      <Link to="/" className="mt-6 inline-flex items-center gap-1 text-xs font-semibold text-primary">See the platform <ArrowRight className="h-3 w-3" /></Link>
                    </div>
                  </div>
                </NavigationMenuContent>
              </NavigationMenuItem>
              <NavigationMenuItem><Link to="/pricing" className={navigationMenuTriggerStyle()}>Pricing</Link></NavigationMenuItem>
              <NavigationMenuItem><NavigationMenuTrigger>Resources</NavigationMenuTrigger><NavigationMenuContent><MenuGroup items={RESOURCES} /></NavigationMenuContent></NavigationMenuItem>
        ...
        <div className="hidden items-center gap-2 md:flex">
          <Button asChild variant="ghost" size="sm"><Link to="/docs">Explore docs</Link></Button>
          <Button asChild size="sm"><Link to="/contact">Request a Demo <ArrowRight className="h-3.5 w-3.5" /></Link></Button>
        </div>
        <button type="button" onClick={() => setOpen((v) => !v)} aria-label={open ? "Close menu" : "Open menu"} aria-expanded={open} className="inline-flex h-10 w-10 items-center justify-center rounded-lg border border-border text-foreground md:hidden">
